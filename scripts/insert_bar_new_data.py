    ...
    try:
        ...
        async for session in get_session():
            try:
                ...
                for row in data:
                    start_time = time.time()  # Start timing for processing a bar
                    try:
                        ...
                        print(e)
                    # finally:
                    #     end_time = time.time()  # End timing for processing a bar
                    #     print(f"Time taken to process bar for date {row['日付']}: {end_time - start_time:.4f} seconds")
